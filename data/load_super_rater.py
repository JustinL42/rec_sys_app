    ...
    with conn:
        with conn.cursor() as cur:
            for row, index in zip(rows, range(len(rows))):
                ...
                authors = re.sub(r"\\`", r"", 
                    re.sub(r"\\'", r"", rating_and_authors[3:]))
                ...
                try:
                    ...
                    for one_title_id in title_id:
                        rating_list, year, original_id_set = \
                            ratings_dict.get(one_title_id, ([], None, set()))
                        rating_list.append(rating)
                        original_id_set.add(
                            '"{}" by {}'.format(title, author_str))
                        ratings_dict[one_title_id] = \
                            (rating_list, index, original_id_set)
                ...
                rating_list, year, original_id_set = \
                    ratings_dict.get(r_id, ([], None, set()))
                rating_list.append(rating)
                ...
                for secondary_result in results[1:]:
                    if title.lower() == secondary_result[1].lower() and \
                        author_set == set([a for a in \
                            secondary_result[2].lower().split(', ')] ):
                ...
                elif exact_title_match and \
                    exact_author_match and not only_exact:
            ...
            if not DRY_RUN:
                cur.execute("""
                    ...
                    RETURNING ID;
                """, ("Dave", "Dave", last_name, password, email, 
                        is_active, is_superuser, is_staff, 
                        date_joined, virtual)
                )
            ...
            for book_id, (rating_list, _, original_id_set) in \
                sorted(ratings_dict.items(), key=lambda x : -x[1][1]):
