...
from django.db.models import Avg, Case, CharField, F, Q, Value, When
...
def general_book_search(search):
    ...
    books = (
        ...
        .annotate(
            exact_match=Case(
                When(title__lower=search, then=Value(1) + F("editions")),
                default=Value(0),
            )
        )
        .annotate(
    ...
    return select_bookrow_values(books).order_by(
        "-exact_match", "-rank", "title_id"
    )


def joined_to_ratings(books, user_id):
    return books.annotate(
        rating_score=Avg("rating__rating", filter=Q(rating__user=user_id)),
        rating_saved=BoolAnd("rating__saved", filter=Q(rating__user=user_id)),
        rating_blocked=BoolAnd(
            "rating__blocked", filter=Q(rating__user=user_id)
        ),
